...
def load_pickle(f): 
    ...
    # 파이썬 버전에 따라 pickle을 로드하는 방식이 달라진다
    if version[0] == '2':
        return  pickle.load(f)
    elif version[0] == '3':
        return  pickle.load(f, encoding='latin1')
    
    # 파이썬3는 직렬화된 파이썬 2 스트링을 (바이트로 변환하려는 의도와는 다르게) 스트링 오브젝트로 변환하게 된다.  
    # 파이썬3는 이를 아스키 코덱을 통해 하게 되는데 이 코덱은 모든 256 8비트 캐릭터를 지원하지 않기 때문에 예외를 발생시키게 된다.
    # 이때 모든 256 캐릭터를 지원하는 latin-1 인코딩을 이용하면 이러한 문제를 피할 수 있으며 스트링을 바이트로 인코딩하게 되는 것이다. 
...
def load_CIFAR10(ROOT):
  ...
  xs = []
  ys = []
  for b in range(1,6): # 5회 반복
    f = os.path.join(ROOT, 'data_batch_%d' % (b, )) # dir/data_batch_1, data_batch_2, data_batch_3, data_batch_4, data_batch_5
